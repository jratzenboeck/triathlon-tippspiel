  ...
  <div>
    ...
    <div v-else class="bg-white rounded-lg shadow-sm border">
      <div v-for="(entry, i) in entries" :key="entry.user_id"
        :class="['flex items-center justify-between px-4 py-3 border-b last:border-0', entry.user_id === auth.user?.id ? 'bg-indigo-50' : '']">
        ...
        <div class="flex items-center gap-4 text-sm text-gray-500">
          ...
          <span class="font-semibold text-gray-900 w-16 text-right">{{ entry.points }} {{ $t('common.points') }}</span>
        </div>
      </div>
    </div>
  </div>
...
onMounted(async () => {
  const { data } = await supabase.from('bets').select('user_id, points, profiles!inner(display_name)')

  const scores = {}
  for (const b of data || []) {
    if (!scores[b.user_id]) {
      scores[b.user_id] = { user_id: b.user_id, display_name: b.profiles.display_name, points: 0, bets: 0 }
    }
